    ...
    <aside className={cn(
      ...
      <div className="px-5 py-5 border-b border-gray-50">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-[#4556e0] rounded-xl flex items-center justify-center text-white font-bold text-xs">CSM</div>
          <div>
            <div className="text-sm font-bold tracking-tight">Team Tracker</div>
            <div className="text-[10px] text-gray-400 font-medium">{profile?.role === 'admin' ? 'Leadership Dashboard' : profile?.name || 'CSM'}</div>
          </div>
        </div>
      </div>
          ...
          <div key={group.label} className="mb-3">
            ...
            {group.items.map((item) => {
              ...
              return (
                ...
                >
                  <svg className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 2.5 : 2} strokeLinecap="round" strokeLinejoin="round">
                    <path d={item.icon} />
                  </svg>
                  {item.label}
                </Link>
